	...
	function readFONT(data, offset, idx, tmap, out) {
		...
		if(fvar) {
			for(var i=0; i<fvar[1].length; i++) {
				...
				if(fv[3]==null) fv[3]=(name["fontFamily"]+"-"+name["fontSubfamily"])["split"](" ")["join"]("");
...
Typr["U"] = function() {
	...
	function glyphToPath(font, gid, noColor,axs) {
		...
		var SVG = font["SVG "], CFF = font["CFF "], COLR=font["COLR"], CBLC=font["CBLC"], CBDT=font["CBDT"], sbix=font["sbix"], upng=typeof UPNG !== "undefined" ? UPNG : null;
	...
	function pathToContext(path, ctx) {
		...
		for(var j=0; j<cmds.length; j++) {
			...
			if     (cmd=="M") {
				...
				var upng=typeof UPNG !== "undefined" ? UPNG : null;
		...
		.then(function (res) {
			...
			Typr["U"]["shapeHB"] = (function () {
				...
				return function (fnt, str, prm) {
					...
					if(te==null) te = new TextEncoder("utf8");
